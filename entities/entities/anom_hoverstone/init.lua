...
function ENT:PhysicsSimulate(phys, delta)
	if !self.Active then return SIM_NOTHING end;
...
function ENT:Think()
	...
	for _, ply in pairs(player.GetAll()) do
		if ply:GetPos():Distance(self.Entity:GetPos()) < 2000 then
			active = true;
		end
	end
...
function ENT:PhysicsCollide(data, phys)
	...
	if CheckPly(ent) and CheckChar(ent) then
		...
		if suit != "" then
			cond = math.Clamp(math.Round(cond - (10 / suitData.Durability), 1), 0, 100);
