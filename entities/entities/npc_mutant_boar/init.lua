...
ENT.idle1 = Sound("hgn/stalker/creature/boar/boar_idle_1.wav")
ENT.idle2 = Sound("hgn/stalker/creature/boar/boar_idle_2.wav")
ENT.idle3 = Sound("hgn/stalker/creature/boar/boar_idle_3.wav")
ENT.idle4 = Sound("hgn/stalker/creature/boar/boar_idle_4.wav")
...
ENT.model1 = "models/stalkertnb/boar1.mdl"
ENT.model2 = "models/stalkertnb/boar1.mdl"
...
function ENT:Think()
	...
	local function Attack_Melee()
		...
		if entstoattack != nil then
			for _,v in pairs(entstoattack) do
				if ( (v:IsNPC() or ( v:IsPlayer() and !v:GetEntry("Observing") and v:Alive() and self:Disposition(v) == 1)) and (v != self) and (v:GetClass() != self:GetClass()) or (v:GetClass() == "prop_physics")) then
					if CheckPly(v) and CheckChar(v) then
						local damage = self.Damage;
						local dampener = 0;
						local suit, cond = ParseDouble(v:GetEntry("Suit"));
						if suit != "" then
							...
							v:UpdateEntry("Suit", suit .. ";" .. cond);
						end
						local acc = v:GetEntry("Acc");
						if acc != "" then
							local accData = BASH.Items[acc];
							dampener = dampener + accData.BodyArmor;
						end
						local arts = v:GetEntry("Artifacts");
						...
						damage = damage - ((dampener / damage) * damage);
						v:TakeDamage(damage, self.Entity);
					else
    ...
    if GetConVarNumber("ai_disabled") == 0 then
		...
		if self:GetEnemy() != nil then
			if self:GetEnemy():IsPlayer() and self:GetEnemy():GetEntry("Observing") then
				self:AddEntityRelationship( self:GetEnemy(), 2, 10 )
				self:ClearEnemyMemory()
				self:SetEnemy(nil);
				self:FindEnemyDan();
				self.MeleeAttaching = false;
				return;
			end
...
function ENT:FindEnemyDan()
	...
	for k,v in pairs(MyNearbyTargets) do
		if v:IsPlayer() and v:GetEntry("Observing") then continue end;
		if v:IsPlayer() and self:Disposition(v) != 1 then
...
function ENT:FindCloseEnemies()
	...
	if (!MyNearbyTargets) then print( "No Targets!" ); return end
	for k,v in pairs(MyNearbyTargets) do
		if (!v:IsPlayer() and v:Disposition(self) == 1) || (v:IsPlayer() and !v:GetEntry("Observing") and self:Disposition(v) == 1) then
		print(v:GetClass())
...
function ENT:HasLOS()
	...
	if self:GetEnemy():IsPlayer() and self:GetEnemy():GetEntry("Observing") then return false end;
	local tracedata = {}
...
function ENT:KilledDan()
	...
	if math.random(10) > 5 then
		...
		pos.z = pos.z + 5;
		local trophy = ents.Create("bash_item");
		trophy:SetItem("trophy_boar");
		trophy:SetPos(pos);
		trophy:SetAngles(Angle(0, 0, 0));
		trophy:Spawn();
		trophy:Activate();
	end
